...
PageJsonInit.pages.forEach((el: any) => {
  const customType: pagesCustomType = <pagesCustomType>(
    (el?.style?.navigationStyle ?? 'default')
  )
  const bg
    ...
    || '#FFFFFF'
  const txtColor
    = (el.style?.navigationBarTextStyle
      ?? PageJsonInit?.globalStyle?.navigationBarTextStyle
      ?? 'black')
    || 'black'
  pages.push({
    path: el.path,
  ...
  PageJsonInit?.subPackages.forEach((el: any) => {
    ...
    el.pages.forEach((el2: any) => {
      ...
      const bg
        ...
        || '#FFFFFF'
      const txtColor
        = (el2.style?.navigationBarTextStyle
          ?? PageJsonInit?.globalStyle?.navigationBarTextStyle
          ?? 'black')
        || 'black'
      pages.push({
...
let $tm: tmUtil = {
  ...
  pages,
  globalNavStyle:
    (PageJsonInit?.globalStyle.navigationStyle as navigationStyle) ?? 'default',
  isOpenDarkModel: (
    PageJsonInit?.globalStyle?.navigationBarBackgroundColor ?? ''
  ).includes('@'),
