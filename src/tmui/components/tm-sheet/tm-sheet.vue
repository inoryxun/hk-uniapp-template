...
const tmcomputed = computed<cssstyle>(() => {
  ...
  const _props_rs = uni.$tm.u.deepClone(props)
  const customPropsDefault = uni.$tm.u.deepObjectMerge(_props_rs, { blur: _blur.value, text })
...
const c_w = computed(() => {
  let w = Number.parseFloat(String(_width.value)) - Number.parseFloat(String(props.padding[0]))
  // #ifndef APPNVUE
  w = w - Number.parseFloat(String(props.border)) * 2
  // #endif
  // #ifdef APPNVUE
  w = w - Number.parseFloat(String(props.border))
  // #endif
  return w
})
const c_h = computed(() => {
  let h = Number.parseFloat(String(_height.value)) - Number.parseFloat(String(props.padding[1]))
  // #ifndef APPNVUE
  h = h - Number.parseFloat(String(props.border)) * 2
  // #endif
  // #ifdef APPNVUE
  h = h - Number.parseFloat(String(props.border))
  // #endif
  return h
})
// 设置响应式主题文字色。
...
<template>
  <view
    ...
    :blur-effect="_blurEffect"
    class="flex flex-col noNvueBorder" :class="[
      parenClass_p,
  ...
  >
    <view
      ...
      class="flex noNvueBorder flex-col flex-1" :class="[customClass]"
      :style="contStyle_p"
    >
      <slot />
    </view>
