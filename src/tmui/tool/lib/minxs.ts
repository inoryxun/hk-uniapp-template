...
import { borderStyle, cssDirection } from './interface'
...
// 暗黑状态。
export function computedDark(props: any, tmcfg: tmVuetify): boolean {
...
// 自定义类计算属性。
export function computedClass(props: any): string {
...
// 自定义样式计算属性。
export function computedStyle(props: any): object {
...
// 主题样式表
export function computedTheme(props: any, dark: boolean, store: any): cssstyle {
  ...
  let theme = new colors.themeColors(store.colorList)
  ...
  const c = theme.getTheme({
    ...
    dark,
    borderWidth: border,
    shadow: Number.parseInt(String(shadow)),
    round: Number.parseInt(String(round)),
    outlined: !!outlined,
    text: !!text,
    borderStyle,
