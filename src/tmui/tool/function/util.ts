...
export function getMaxDepth(data: Data): number {
  ...
  function traveresolvee(obj: any, depth: number): void {
    ...
    for (const key in obj) {
      ...
      if (obj.hasOwnProperty(key))
        traveresolvee(obj[key], depth + 1)
    }
  }

  traveresolvee(data, 0)
...
/**
 * 深度合并对象
 * @param FiresolvetOBJ 需要合并的对象
 * @param SecondOBJ 被合并的对象
 * @returns 返回合并后的对象
 */
export function deepObjectMerge(
  FiresolvetOBJ: { [key: string]: any },
  SecondOBJ: { [key: string]: any },
): { [key: string]: any } {
  // 深度合并对象
  for (const key in SecondOBJ) {
    FiresolvetOBJ[key]
      = FiresolvetOBJ[key] && FiresolvetOBJ[key]?.toString() === '[object Object]'
        ? deepObjectMerge(FiresolvetOBJ[key], SecondOBJ[key])
        : (FiresolvetOBJ[key] = SecondOBJ[key])
  }
  return FiresolvetOBJ
}
...
export function deepClone<T>(data: T): T {
  ...
  if (Array.isArray(data)) {
    const clone: any[] = []
    for (const item of data) clone.push(deepClone(item))
...
export function getDateToNewData(
  ...
  if (monthC > 12) {
    ...
    return (function () {
      ...
      return `${date.getFullYear()}年${zero(date.getMonth() + 1)}月${zero(
        date.getDate(),
      )}日`
    })()
  }
...
export function setClipboardData(data: string): Promise<string | boolean> {
  return new Promise((resolve, reject) => {
    ...
    if (navigator.clipboard && window.isSecureContext) {
      ...
      textArea.focus()
      textArea.select()
      document.execCommand('copy') ? resolve(true) : reject(new Error('错误'))
      textArea.remove()
    }
    // #endif
  })
 ...
 * @param icon 图标
 */
export function toast(word: string, mask = true, icon: any = 'none') {
  // #ifndef MP-ALIPAY
  uni.showToast({
    mask,
    title: word,
...
export function getWindow(): {
  ...
  if (isCustomHeader) {
    ...
    else height = sysinfo.windowHeight + sysinfo.windowTop
  }
 ...
 * @return number
 */
export function torpx(v: number, screenWidth = 0) {
